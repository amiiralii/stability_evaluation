...
treatments = ["gini", "entropy"]

performace_error = {}
error_dist = {}
for imp in treatments:
    the.Impurity = imp
    mse = 0
    error = []
    ...
    error_dist[imp] = error
    performace_error[imp] = (mse / repeats) ** 0.5
...
for imp in treatments:
    ...
    for idx, row in enumerate(test.rows):
        ...
        preds = adds( outputs )
        stability_comp[idx][imp] = preds.sd
        if preds.sd < 0.35 * b4_wins.sd:
            aggreement += 1

    stability_aggreement[imp] = (aggreement * 100) // tests_size
