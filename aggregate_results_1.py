"""
Aggregate CSV results from results/1.3/ directory.

Each per-dataset CSV has columns:
  trt, performance_error, stability_agreement, best_performance, best_stability
where trt = budget number (10, 20, 50, 100, 200).

Output: 4 pivot tables in results/1.3/aggregates/
  aggregated_performance_error.csv
  aggregated_stability_agreement.csv
  aggregated_best_performance.csv
  aggregated_best_stability.csv

Each pivot table has datasets as rows and budget treatments as columns.
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="Aggregate 1.3.py per-dataset CSVs into pivot tables."
    )
    ap.add_argument(
        "--input", default="results/1.3",
        help="Directory containing per-dataset CSVs (default: results/1.3)"
    )
    ap.add_argument(
        "--output", default=None,
