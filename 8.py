"""
8.py — EZR (standard tree) vs Causal EZR (causal tree): Stability & Performance comparison.

Both treatments run under identical conditions
(same splits, same seeds, same budget = 50, same likely() labeled rows).

Treatments: ezr, causal_ezr

  ezr:        Tree()        from ezr.py   — correlation-based splits
  causal_ezr: causalTree()  from causal_tools.py — causality-based splits
                             (H(Y|X)/H(Y) ratio + confounder removal)
...
Output: one CSV per dataset under results/8/{dataset}.csv
...
def run(file_directory, out_dir="results/8", repeats=20):
    ...
    for trt in TREATMENTS:
        ...
        for rand_seed in range(repeats):
            ...
            # Both treatments use the same labeled rows chosen by likely()
            labels  = likely(train)
    ...
    for rand_seed in range(repeats):
        ...
        # causal_ezr: causal tree
        sampled_data = clone(all_data, sampled)
        causal_models.append((sampled_data, causalTree(sampled_data)))
...
if __name__ == "__main__":
    ...
    output_dir     = sys.argv[2] if len(sys.argv) > 2 else "results/8"
